...
- Dev Vite services accept Tailscale hostnames through `__VITE_ADDITIONAL_SERVER_ALLOWED_HOSTS=${DASHBOARD_DEV_ALLOWED_HOST:-.tailf1878c.ts.net}` on the shell and all five remote containers. The shell's Vite 8 applies the hook natively; each Vite 5 remote mounts the dashboard-shell-owned `scripts/vite-tailnet-allowed-hosts.config.mjs` read-only into its project root and starts Vite with `--config`, and the wrapper loads the remote's own `vite.config.ts` with its own installed Vite before appending the single pattern to `server.allowedHosts` (preserving `true` and existing entries). The leading dot permits the base tailnet domain and any node hostname under it, so node renames within the tailnet need no change; LAN IPv4/IPv6 literals remain accepted by Vite automatically. The override is one allowed-host pattern and is only needed for a different trusted DNS suffix or hostname.
